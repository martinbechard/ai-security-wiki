...
The [August 30 leaf update watch source](../../../raw/processed/2026-08-30/ai-security-wiki-leaf-update-watch-20260831T000130Z.json) records visibly updated [Google Cloud MCP AI security and safety documentation](https://docs.cloud.google.com/mcp/ai-security-safety) and [Google Workspace MCP security configuration](https://developers.google.com/workspace/guides/configure-mcp-security). The local control remains tool authorization. MCP deployments should preserve:

- risk classification for reversible and irreversible delegated user actions;
- Model Armor-style screening before high-risk tool calls;
- clear logging choices because full prompt or response logging can become a secondary sensitive-data store;
- data-residency evidence when MCP prompt, response, or inspection payloads cross Google Cloud, Workspace, and Model Armor boundaries;
- audit evidence proportionate to the tool's blast radius.
...
- Affected boundary: Google Cloud and Google Workspace MCP tools, `tool.name` permission conditions, allow policies, deny policies, Cloud Logging IAM-aware responses, audit logging, least-privilege agent access, and mitigation evidence for reversible or irreversible delegated tool calls.
...
- Which irreversible MCP tool calls require human approval in addition to IAM conditions and deny policies?
...
- Updated on 2026-08-30 from the [August 30 leaf watcher](../../../raw/processed/2026-08-30/ai-security-wiki-leaf-update-watch-20260831T000130Z.json) with delegated-action mitigation evidence from Google Cloud MCP safety documentation.
