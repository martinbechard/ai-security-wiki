...
- Exploit or incident status: public vulnerability database entry and maintainer advisory corroboration; no active exploitation was identified in the raw source.
- Mitigation state: fixed in 2.10.0-lts according to the raw source; restrict MCP tool import permissions, block `stdio` transport from untrusted imports, and review existing imported tools for command payloads.
...
- Added from the July 23, 2026 raw collector artifact; preserve the distinction between in-window vulnerability database signal and older maintainer advisory evidence.
