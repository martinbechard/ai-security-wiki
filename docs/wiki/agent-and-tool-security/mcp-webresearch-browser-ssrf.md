...
Prompt injection can steer the model-controlled URL argument and turn a browsing tool into server-side request forgery. The relevant local security boundary is the browser tool's network reachability and returned content, covered by [agent network egress controls](agent-network-egress-controls.md), not the broad mcp-webresearch catalog entry, which belongs upstream.
...
- Exploit or incident status: public CVE and NVD record; exploitability is tied to prompt or tool-argument influence.
...
- Added from the July 22, 2026 raw collector artifact and live npm metadata.
