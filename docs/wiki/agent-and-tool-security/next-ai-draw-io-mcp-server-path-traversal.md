...
The [npm package metadata](https://www.npmjs.com/package/@next-ai-drawio/mcp-server) checked during ingest showed `@next-ai-drawio/mcp-server` versions newer than the issue's affected 0.1.16 line, but the raw source did not identify the first fixed version. The [July 23 leaf update watch source](../../../raw/processed/2026-07-23/ai-security-wiki-leaf-update-watch-2026-07-23T200300-0400.json) adds NVD classification as CWE-22, CISA-ADP CVSS 7.8, and SSVC exploitation none. The local security boundary is [agent tool filesystem path containment](../infrastructure-and-supply-chain/agent-tool-filesystem-path-containment.md) for MCP tools.
...
- Exploit or incident status: public CVE and GitHub issue; July 23 NVD enrichment records SSVC exploitation none.
...
- Confidence: medium-high for path traversal mechanics from NVD and the linked issue; medium for remediation because the first fixed package version was not verified.
...
- Added from the July 22, 2026 raw collector artifact, linked GitHub issue, and package metadata checks; enriched from the [July 23 leaf watcher](../../../raw/processed/2026-07-23/ai-security-wiki-leaf-update-watch-2026-07-23T200300-0400.json) with NVD CWE/CVSS and exploitation-status evidence.
