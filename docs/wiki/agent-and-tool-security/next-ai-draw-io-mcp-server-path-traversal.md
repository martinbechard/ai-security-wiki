...
The [npm package metadata](https://www.npmjs.com/package/@next-ai-drawio/mcp-server) checked during ingest showed `@next-ai-drawio/mcp-server` versions newer than the issue's affected 0.1.16 line, but the raw source did not identify the first fixed version. The local security boundary is [agent tool filesystem path containment](../infrastructure-and-supply-chain/agent-tool-filesystem-path-containment.md) for MCP tools.
...
- Exploit or incident status: public CVE and GitHub issue; no live exploitation report was identified in the raw source.
